  ...
  return (
    <ul className="flex flex-wrap gap-2">
      {items.map((item) => (
        <li
          key={item}
          className="flex items-center space-x-1.5 text-[11px] text-zinc-400 font-mono px-2.5 py-1 rounded-full border border-white/[0.08] bg-white/[0.03]"
        >
...
export default function ExportPanel({
  ...
  return (
    <div className="rounded-2xl border border-white/[0.08] bg-[#121216]/90 px-4 sm:px-5 py-4 space-y-4">
      {isReady && <ActionsReceipt watermarkFree={effectiveWatermarkFree} />}

      <div className="flex items-center space-x-3">
        <button
          onClick={onReEdit}
          className="px-4 py-2.5 rounded-full text-xs font-medium border transition-all text-zinc-400 hover:text-white border-white/10 hover:border-white/20 bg-white/[0.04] cursor-pointer"
        >
        ...
        {showUpgradeLink ? (
          ...
          isReady && (
            <button
              ...
              onClick={onDownload}
              className="flex-1 py-2.5 px-5 rounded-full text-xs font-bold transition-all duration-300 flex items-center justify-center space-x-2 group/btn disabled:cursor-not-allowed disabled:opacity-70 bg-gradient-to-r from-amber-300 via-amber-400 to-amber-500 text-[#241a03] shadow-cf-pill hover:brightness-110 hover:shadow-cf-pill-lg cursor-pointer"
            >
